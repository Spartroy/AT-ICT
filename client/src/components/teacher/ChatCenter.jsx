...
const ChatCenter = () => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/chat/teacher/students', {
        headers: {
  ...
  const fetchMessages = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/chat/conversations/${selectedStudent.id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
