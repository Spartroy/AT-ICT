...
const MaterialsCenter = () => {
  ...
  const handleUploadMaterial = async (e) => {
    ...
    try {
      ...
      const url = editingMaterial 
        ? `http://localhost:5000/api/teacher/materials/${editingMaterial._id}`
        : 'http://localhost:5000/api/teacher/materials';
  ...
  const handleDeleteMaterial = async (materialId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/teacher/materials/${materialId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const downloadMaterial = async (material) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/teacher/materials/${material._id}/download`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
                  ...
                  <div className="border-2 border-dashed border-gray-600 rounded-xl p-4 text-center hover:border-blue-500 transition-colors">
                    ...
                    >
                      {editingMaterial && !formData.thumbnail && editingMaterial.thumbnailUrl ? (
                        <div className="flex items-center space-x-2">
                          <img 
                            src={`http://localhost:5000${editingMaterial.thumbnailUrl}`}
