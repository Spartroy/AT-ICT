...
const StudentManagement = () => {
  ...
  const updateAssignmentFeedback = async (assignmentId, studentId, score, feedback) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/teacher/assignments/${assignmentId}/students/${studentId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const downloadStudentFile = async (assignmentId, studentId, filename, originalName) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/assignments/${assignmentId}/submissions/${studentId}/download/${filename}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const downloadQuizFile = async (quizId, studentId, filename, originalName) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/quizzes/${quizId}/submissions/${studentId}/download/${filename}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const updateQuizFeedback = async (quizId, studentId, score, feedback) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/teacher/quizzes/${quizId}/students/${studentId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
