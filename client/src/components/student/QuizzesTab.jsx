...
const QuizzesTab = () => {
  ...
  const startQuiz = async (quizId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/student/quizzes/${quizId}/start`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const submitQuiz = async (quizId) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/student/quizzes/${quizId}/submit`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
